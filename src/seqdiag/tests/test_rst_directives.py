...
def setup_directive_base(func):
    def _(self):
        klass = directives.SeqdiagDirectiveBase
        docutils.register_directive('seqdiag', klass)
        func(self)
...
class TestRstDirectives(unittest2.TestCase):
    ...
    def test_base_with_filename(self):
        ...
        filename = os.path.join(dirname, 'diagrams/node_notes.diag')
        text = ".. seqdiag:: %s" % filename
        doctree = publish_doctree(text)
